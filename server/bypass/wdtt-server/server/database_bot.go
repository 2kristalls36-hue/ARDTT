...
func botLoop(token string, adminIDstr string, wgDev *device.Device) {
  ...
  "password": "%s"
						...
						fileName := fmt.Sprintf("wdtt_%s.conf", pass)
			...
			if waitingForHash {
				...
				if targetPassword == "main" {
					...
					nameEsc := neturl.QueryEscape(fmt.Sprintf("WDTT - Main (%s)", srvIP))
					peerEsc := neturl.QueryEscape(srvIP)
					hashesEsc := neturl.QueryEscape(hash)
					passEsc := neturl.QueryEscape(db.MainPassword)
					wdttLink := fmt.Sprintf("wdtt://config?name=%s&peer=%s&hashes=%s&workers=9&port=9000&pass=%s", nameEsc, peerEsc, hashesEsc, passEsc)

					msgText := fmt.Sprintf("🔗 *Ссылка для главного пароля:*\n`%s`\n\n🔗 *Быстрая ссылка WDTT:* `%s`", link, wdttLink)
  ...
  "password": "%s"
					...
					fileName := fmt.Sprintf("wdtt_main_%s.conf", srvIP)
					sendTelegramFile(token, adminID, fileName, []byte(configJSON))
					continue
				}
				...
				msgText := fmt.Sprintf("👤 Имя: *%s*\n🔑 Новый пароль:\n`%s`\n\n⏰ Действует %d дн. (до %s)\n📱 Лимит: %d устройств\nОжидает первого подключения\n\n🔗 *Быстрая ссылка WDTT:* `%s`\n\n🔗 *Legacy ссылка:* `%s`", newLabel, newPass, tempDays, expDate, tempMaxDevs, wdttLink, link)
				sendTelegram(token, adminID, msgText, nil)
  ...
  "password": "%s"
				...
				fileName := fmt.Sprintf("wdtt_%s.conf", newPass)
				sendTelegramFile(token, adminID, fileName, []byte(configJSON))
				continue
			}
