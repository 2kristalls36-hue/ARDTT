...
Продуктовое имя — **ARDTT**.  
Идентификаторы в коде: пакет `com.ardtt.app`, каталог на VPS `/opt/ardtt`, контейнеры `ardtt-*`, переменные `ARDTT_*`. Установщик при обновлении переносит старый `/opt/nonamevpn`, если он ещё есть.

Знак приложения — стилизованная монограмма **ARDTT**: белое **AR** над бирюзовым **DTT** на тёмно-синем скруглении.
...
- Иконка: белое **AR** над бирюзовым **DTT** (cyan→teal) на тёмно-синем поле (`#031D3B`).
