...
Телефон (админ)
  ...
  SSH (пароль или PEM)
      ...
      │  env ARDTT_PUBLIC_HOST=… ARDTT_GIT_REF=v0.5.239 bash install.sh
      ▼
...
ARDTT_DEPLOY_VERSION='1.0.36' ARDTT_GIT_REF='v0.5.239' \
...
ARDTT_ROLE=exit ARDTT_PUBLIC_HOST='2.26.125.160' ARDTT_DEPLOY_VERSION='1.0.36' \
  ARDTT_GIT_REF='v0.5.239' ARDTT_AUTO_PORTS=1 bash /opt/ardtt/install.sh
...
Клонируйте **тег релиза** (`v0.5.239` = клиент 0.5.239 и стек 1.0.36), не скользящий `main`. Публичный репозиторий клонируется без секретов. Пока репозиторий приватный — HTTPS clone с VPS нужен PAT либо SSH-ключ с правом `repo`; с телефона достаточно `GITHUB_RELEASE_READ_TOKEN` во вшитом APK.
