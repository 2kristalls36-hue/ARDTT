...
Версия **стека** (`DEPLOY_VERSION`, сейчас **1.0.20**) независима от `versionName` приложения. Её бампят только когда меняется то, что уезжает на VPS (Compose, `install.sh`, образы сервисов).
...
NVPN_DEPLOY_VERSION='1.0.20' bash /opt/nonamevpn/install.sh
```
...
```bash
# 1) выход (DNS + WARP)
NVPN_ROLE=exit NVPN_PUBLIC_HOST='2.26.125.160' NVPN_DEPLOY_VERSION='1.0.20' \
...
NVPN_ROLE=entry NVPN_CASCADE_ENABLED=1 \
  ...
  NVPN_PUBLIC_HOST='45.129.2.3' NVPN_DEPLOY_VERSION='1.0.20' \
  bash /opt/nonamevpn/install.sh
...
echo 1.0.20 > DEPLOY_VERSION   # или оставить как в репо
...
```bash
export NVPN_PUBLIC_HOST=1.2.3.4 NVPN_DEPLOY_VERSION=1.0.20
bash /opt/nonamevpn/install.sh
```
...
curl -s http://127.0.0.1:9100/health
# ожидается: "ok": true, "deployVersion": "1.0.20"

ss -ulnp | grep -E '51820|56003'
ss -tlnp | grep -E '9100|9200'
```

С телефона: карточка VPS «Онлайн · деплой 1.0.20 · актуален», создание клиента, импорт профиля, Connect.
