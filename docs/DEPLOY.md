...
Версия **стека** (`DEPLOY_VERSION`, сейчас **1.0.15**) независима от `versionName` приложения. Её бампят только когда меняется то, что уезжает на VPS (Compose, `install.sh`, образы сервисов).
...
NVPN_DEPLOY_VERSION='1.0.15' bash /opt/nonamevpn/install.sh
```
...
echo 1.0.15 > DEPLOY_VERSION   # или оставить как в репо
...
```bash
export NVPN_PUBLIC_HOST=1.2.3.4 NVPN_DEPLOY_VERSION=1.0.15
...
curl -s http://127.0.0.1:9100/health
# ожидается: "ok": true, "deployVersion": "1.0.15"

ss -ulnp | grep -E '51820|56003'
ss -tlnp | grep -E '9100|9200'
```

С телефона: карточка VPS «Онлайн · деплой 1.0.15 · актуален», создание клиента, импорт профиля, Connect.
