...
Версия **стека** (`DEPLOY_VERSION`, сейчас **1.0.25**) независима от `versionName` приложения. Её бампят только когда меняется то, что уезжает на VPS (Compose, `install.sh`, образы сервисов).
...
NVPN_DEPLOY_VERSION='1.0.25' bash /opt/nonamevpn/install.sh
```
...
```bash
# 1) выход (DNS + WARP)
NVPN_ROLE=exit NVPN_PUBLIC_HOST='2.26.125.160' NVPN_DEPLOY_VERSION='1.0.25' \
...
NVPN_ROLE=entry NVPN_CASCADE_ENABLED=1 \
  ...
  NVPN_PUBLIC_HOST='45.129.2.3' NVPN_DEPLOY_VERSION='1.0.25' \
  bash /opt/nonamevpn/install.sh

# 3) ключ входа → /opt/nonamevpn/stack/data/cascade.peer.pub на выходе
```

Повторный «Обновить деплой» **входа** без `NVPN_CASCADE_ENABLED=1` больше не сбрасывает живой каскад: `install.sh` копирует флаги из `stack/.env` и `data/cascade.peer.endpoint`. Снять каскад явно: `NVPN_CASCADE_FORCE_DISABLE=1`.

На standalone-входе `NVPN_CASCADE_DNS` в `.env` должен быть пустым. Иначе provision отдаёт профилям `10.10.0.2`, хотя `nvpn-cascade` не запущен и DNS на `10.8.0.1` / `10.9.0.1`.
...
```bash
export NVPN_PUBLIC_HOST=1.2.3.4 NVPN_DEPLOY_VERSION=1.0.25
bash /opt/nonamevpn/install.sh
```
...
curl -s http://127.0.0.1:9100/health
# ожидается: "ok": true, "deployVersion": "1.0.25"
