...
| `ArdttNavigationBar` | нижняя панель: оболочка без тени и без плашки выбранной вкладки; на Android 13+ край преломляет фон | `AppRoot` |
...
На Android 13 и новее эти прозрачные плашки и стеклянные кнопки преломляют
то, что за ними: край искривляет кадр, центр плотнее (у кнопки по-прежнему 80%
её цвета). Листы и диалоги остаются непрозрачными, шапка растворяется только
альфой, без размытия. До Android 13 заливка ровная, как раньше. Поиск обхода
при открытой клавиатуре — непрозрачное поле, без преломления.
