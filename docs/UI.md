...
| Диагностика | admin | `ArdttFeedScaffold` на хабе; при открытом инструменте `ArdttScrollChrome` | по умолчанию открыта «Сеть»; без подписи «Сеть и журнал»; «Сеть» и «Журнал» с обычным отступом шапки и ленты, «Журнал» сразу под картой, не у плашки; кнопки журнала в шапке терминала | — |
