...
| Профили | оба | `ArdttLazyFeedScaffold` + sticky «Добавить» | карточка как у сервера: название → срок → ⋮, адреса слева / «● Активен» справа, трафик снизу; выбрать (radio), ⋮ подключить/копировать/поделиться/переименовать/удалить | `ProfileAddSheet`, подписка / ручной ввод / переименование, `ArdttConfirmDialog` удаления, `ProfileShareDialog` |
