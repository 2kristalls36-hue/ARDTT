...
с GPU `BlurEffect`; альфа-маска `DstIn` гасит сам blur к низу полосы, а не
отдельную цветную подложку. Отступ содержимого = высота шапки + fade, поэтому
первая строка при нулевой прокрутке не лежит под переходом. Касания
перехватывает только чёткая шапка, не полоса fade. Заголовок и status/cutout —
отдельный чёткий слой сверху. На API 28–30 blur пропускается, остаются fade и
тематическая подложка. Скрытые под панелью строки не кликаются и не
дублируются в TalkBack.
