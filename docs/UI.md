...
| `ArdttPrimaryButton` | полноширинный sticky CTA: свой цвет на 80% в светлой и тёмной теме, обводка плашки вкладок, без тени | Туннель, Профили, Серверы, Клиенты, Тестирование |
...
| `ArdttNavigationBar` | нижняя панель: оболочка без тени и без плашки выбранной вкладки; на Android 13+ фон за плашкой преломляется, включая текст | `AppRoot` |
...
| Профили | оба | `ArdttLazyFeedScaffold` + sticky «Добавить» | карточка как список серверов: название и срок в колонке, ⋮ снаружи как шеврон (компактная иконка), адреса / «● Активен», трафик; выбрать (radio), ⋮ подключить/копировать/поделиться/переименовать/удалить | `ProfileAddSheet`, подписка / ручной ввод / переименование, `ArdttConfirmDialog` удаления, `ProfileShareDialog` |
| Обход | user — вкладка; admin — из Настроек | `ArdttScrollChrome` + плавающий поиск | панель Приложения/Правила (`ArdttChoiceChipRow`), ЧС/БС, системные приложения, «Очистить»; заглушки приложений до низа экрана | подтверждение очистки правил |
| Журнал | user — вкладка; admin — из Диагностики | `ArdttScrollChrome` | Копировать / Расшарить / Удалить в шапке терминала справа, с промежутком между кнопками; терминал без поиска и автопрокрутки, время подключения слева | `ArdttConfirmDialog` |
...
| Диагностика | admin | `ArdttFeedScaffold` на хабе; при открытом инструменте `ArdttScrollChrome` | по умолчанию открыта «Сеть»; у «Сети» и «Журнала» нет второй строки; «Журнал» сразу под картой, не у плашки; кнопки журнала в шапке терминала | — |
...
- Admin-only элементы **скрываются** (`if (admin)`), не блокируются: свитчи «Скрыть
  ...
  «Правила обхода» в Настройках, статус-панель туннеля (статус и текущий режим над
  разделителем, IP провайдера и туннеля — `IpApiLookup.fetchPublicIps`, как на карте
  сети: те же пробы, `DeployHop.warpExitProvisionUrl`, общий кэш хопов и geo-IP;
  карта не перезапрашивает то, что уже опросил Туннель; после «Оператор» — строка
  «Уровень сигнала» (подпись слева, Wi‑Fi и соты справа, цвет по уровню: зелёный /
  жёлтый / красный; у текущего режима черточка снизу); блока «Проверка сети» нет), подробные логи.
- User-only: иллюстрированный туннель с обоями и «Классический вид» (админ всегда на
  панельном туннеле). Обход у пользователя — вкладка, у админа — вложенный маршрут
  из Настроек. Журнал у пользователя — вкладка; у админа журнал и сеть разворачиваются
  на вкладке Диагностика. Тестирование открывается только из Настроек. Содержимое
  инструментов одно и то же.
...
Sticky CTA держит свой оттенок в светлой и тёмной теме, без подмешивания оболочки.
Непрозрачные Primary и Danger заливаются тем же цветом на 80%, уже сведённым с `surface`.
Круглая кнопка питания туннеля сохраняет свою форму; цвета и доступность — из
той же системы. Кнопка текущего профиля на иллюстрированном туннеле — то же
стекло, что у sticky CTA; замок сессии это иконка, не другая заливка.
Полупрозрачная заливка стекла не получает тень: слой тени Material на кадр
рисует её непрозрачной. Питание, бейдж темы, поиск обхода и плашка вкладок
держат ту же оболочку без тени.
На Android 13 и новее эти прозрачные плашки и стеклянные кнопки — линза.
Середина увеличивает то, что за плашкой, узкий скос у кромки сжимает фон вдоль
контура и ловит тонкий блик; размытие слабое, чтобы края букв оставались.
Середина не забеливается. Подпись стеклянной кнопки считается по
фону страницы: на тёмной поверхности она светлая, потому что тёмный шрифт на
тонкой плашке теряется. Оттенок кнопки не смешивается с оболочкой. Листы и
диалоги остаются непрозрачными, шапка растворяется только альфой, без размытия.
До Android 13 заливка ровная, как раньше. Поиск обхода при открытой клавиатуре —
непрозрачное поле, без преломления.
...
`containerColor` вызывающей стороны не переопределяется.
...
Плашка вкладок — та же оболочка, без тени и без заливки выбранного пункта. Текущая вкладка отличается цветом иконки и подписи. Нажатие на вкладку на одну секунду двигает её иконку и возвращает в исходное положение: шестерёнка настроек поворачивается на 180° и обратно, ключ туннеля прокручивается на 360° вокруг горизонтальной оси — в середине он вверх ногами, затем снова в исходной позе, без рывка в начале и в конце. `selectableGroup` + `Role.Tab`.
...
- Логи: терминал без поиска и фильтров; копировать / расшарить / удалить в шапке терминала, с отступом между кнопками.
- Сеть: pull-to-refresh и «Повторить» на карточке ошибки.
- Профили: `ArdttLazyFeedScaffold` + стабильные ключи `id`.
- Метод обхода: «Код сохранён на этом устройстве», затем «Создать код» и «Ввести вручную», затем «Вход выполнен», затем «Завершить».
