...
к низу полосы, кроссфейд в чёткие пиксели. Отступ содержимого = высота шапки +
fade, поэтому первая строка при нулевой прокрутке не лежит под переходом.
Касания перехватывает только чёткая шапка, не полоса fade. Заголовок и
status/cutout — отдельный чёткий слой сверху. На API 28–30 blur пропускается;
контент всё равно уходит в прозрачность. Скрытые под панелью строки не
кликаются и не дублируются в TalkBack.
