...
| Диагностика | admin | `ArdttFeedScaffold` на хабе; при открытом инструменте `ArdttScrollChrome` | по умолчанию открыта «Сеть»; без подписи «Сеть и журнал»; кнопки журнала в шапке терминала | — |
