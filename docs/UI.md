...
| Диагностика | admin | `ArdttFeedScaffold` на хабе; при открытом инструменте `ArdttScrollChrome` | «Сеть» / «Журнал» / «Тестирование»: нажатие поднимает кнопку вверх и показывает содержимое; ранее открытые остаются свёрнутыми сверху, остальные — свёрнутыми под панелью | — |
...
- User-only: иллюстрированный туннель с обоями и «Классический вид» (админ всегда на
  ...
  из Настроек. Журнал у пользователя — вкладка; у админа журнал, сеть и тестирование
  разворачиваются на вкладке Диагностика (тестирование из Настроек остаётся отдельным
  маршрутом). Содержимое инструментов одно и то же.
