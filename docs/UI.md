...
| `ArdttColors` | семантика вне `ColorScheme`: `Connected`/`Warning` (+ `On*`, `*OnLight`/`*OnDark`), `SessionLit`, `Recording`, `PathDirect`/`PathBypass`, support/wallpaper/drone tokens, `Terminal*` |
...
`ServerOsBadgeDefaults`, `ArdttStatusChipDefaults`, `ArdttTerminalCardDefaults`,
`ArdttLabelValueRowDefaults`, `ArdttStateBlockDefaults`) держат разовую геометрию
и интервалы рядом с местом использования, с именем — литерал в вызове недопустим.
