...
| Профили | оба | `ArdttLazyFeedScaffold` + sticky «Добавить» | карточка как у сервера: те же отступы CompactCard, контур по умолчанию, название onSurface, срок — бейдж FillSoft как OS, адреса во внутренней строке / «● Активен», трафик снизу, сверху списка Small как у серверов; выбрать (radio), ⋮ подключить/копировать/поделиться/переименовать/удалить | `ProfileAddSheet`, подписка / ручной ввод / переименование, `ArdttConfirmDialog` удаления, `ProfileShareDialog` |
