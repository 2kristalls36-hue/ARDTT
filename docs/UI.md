...
| `ArdttNavigationBar` | нижняя панель: заливка оболочки без тени и без плашки выбранной вкладки | `AppRoot` |
...
| Профили | оба | `ArdttLazyFeedScaffold` + sticky «Добавить» | карточка как список серверов: название и срок в колонке, ⋮ снаружи как шеврон (компактная иконка), адреса / «● Активен», трафик; при лимите ГБ та же шкала, что у клиента, в нижнем поле карточки (высота не растёт); выбрать (radio), ⋮ подключить/копировать/поделиться/переименовать/удалить | `ProfileAddSheet`, подписка / ручной ввод / переименование, `ArdttConfirmDialog` удаления, `ProfileShareDialog` |
...
Sticky CTA держит этот цвет на 80% в светлой и тёмной теме, без подмешивания оболочки.
Непрозрачные Primary и Danger заливаются тем же цветом, уже сведённым с `surface`, чтобы оттенок совпадал со стеклянной кнопкой.
...
Плашка вкладок — та же оболочка, без тени и без заливки выбранного пункта. Текущая вкладка отличается цветом иконки и подписи. Нажатие на вкладку на одну секунду двигает её иконку и возвращает в исходное положение: шестерёнка настроек поворачивается на 180° и обратно, ключ туннеля плавно переворачивается на 360° вокруг горизонтальной оси — в середине он вверх ногами, затем снова в исходной позе, без рывка в начале и в конце. Значок серверов — стойка с лампочками того же контура, что и остальные иконки; за секунду лампочки дважды мягко гаснут и снова загораются. `selectableGroup` + `Role.Tab`.
...
возвращаются. Исключение — журнал: и на вкладке, и внутри «Диагностики»
название остаётся на месте, пока лента едет вниз. Светлая подложка только в полосе статус-бара, не под заголовком.
