...
| Диагностика | admin | `ArdttFeedScaffold` на хабе; при открытом инструменте `ArdttScrollChrome` | «Сеть» / «Журнал»: нажатие поднимает кнопку вверх и показывает содержимое; ранее открытые остаются свёрнутыми сверху, остальные — под панелью | — |
