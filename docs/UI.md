...
| Журнал | user — вкладка; admin — из Диагностики | `ArdttScrollChrome` | Копировать / Расшарить / Удалить в заголовке страницы; терминал без поиска и автопрокрутки, время подключения слева | `ArdttConfirmDialog` |
...
| Серверы → карточка → Клиенты / Деплой | admin | `ArdttScrollChrome` (+ sticky CTA) | список: «Добавить сервер», экспорт/импорт; обзор: ⋮ на карточке после версии ОС; клиенты / обновить / удалить / деинсталляция; деплой: сохранить / установить / назад | `DeployProgressSheet`, переименование, удаление/переустановка, `ClientSettingsSheet`, лимиты, `ArdttConfirmDialog` отвязки |
| Диагностика | admin | `ArdttFeedScaffold` на хабе; при открытом инструменте `ArdttScrollChrome` | по умолчанию открыта «Сеть»; при открытом «Журнале» кнопки журнала в шапке Диагностики | — |
...
- Admin-only элементы **скрываются** (`if (admin)`), не блокируются: свитчи «Скрыть
  ...
  карта не перезапрашивает то, что уже опросил Туннель; после «Оператор» — инвертированная строка «Сигнал»
  (Wi‑Fi и соты); блока «Проверка сети» нет), подробные логи.
...
- Логи: терминал без поиска и фильтров; копировать / расшарить / удалить в заголовке.
