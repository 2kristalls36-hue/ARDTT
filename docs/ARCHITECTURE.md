...
Android
  ...
  └─ Path B: TUN → WRAP → TURN/TCP → VPS1 bypass (raw0)
              ...
              с каскадом, Hide-IP вкл ──────────► warp0 на VPS1 (prio 300)
...
| WARP | hideIp /32 (prio 300), бьёт каскад | не для клиентов; выход — WAN |
| Если выход мёртв | hop без handshake → `awg0`/`wdttraw0` down, туннель на телефоне падает | — |

Профиль по-прежнему с endpoint VPS1. Снаружи при выключенном Hide-IP виден IP VPS2, при включённом — Cloudflare, не IP входа.
