...
| TURN transport | **TCP** |
| Workers | См. [нагрузка Path B](#нагрузка-path-b-workers) — не оставлять слепой `1` без оговорок |
...
**Предложение (зафиксировать как default):**

| Режим | Workers | Комментарий |
|-------|---------|-------------|
| MVP default | **3** | Компромисс скорость/стабильность на 1 hash, TCP |
| «Экономия» в настройках | 1 | Слабые сети / отладка |
| Потолок | 6–9 | Только если замеры ок; UI можно не светить |
