...
| Переподключение | Мягкий restart транспорта при смене Wi‑Fi/LTE (как WDTT-Plus `AMNEZIA_STYLE_RECOVERY`): VpnService живёт, backend/`libclient` перезапускается после settle ~8 с |
