...
| Переподключение | Wi‑Fi в Auto → сразу Direct; LTE Auto следует зонду :9100 (VPS up → Direct, whitelist → Bypass). `libclient`/звонок держится ~5 мин после ухода на Direct, чтобы возврат на LTE не требовал нового VK-звонка. Settle ~1 с, VpnService живёт |
