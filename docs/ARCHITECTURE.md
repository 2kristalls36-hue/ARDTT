...
| — | VK / TURN | Hash звонка создаётся через VK-логин (1 раз); Connect идёт **анонимным `vkcalls`** по hash. Капча/legacy нет. Мёртвый звонок: спросить / тихий recreate в настройках. **TCP**, **workers=1**. |
...
В qWDTT: **«Звонок»** (`vkcalls`) и **«Капча»** (`legacy`).

**У нас только `vkcalls`.** Legacy/капча не поддерживаем, переключатель дозвона в UI нет.

`vkcalls` — путь **анонимных** TURN-кредов по хешу звонка (без капчи). Аккаунт VK для каждого Connect не нужен.

### Зачем тогда вход в VK

Только чтобы **один раз создать звонок** и получить hash:

```
Первый раз (или нет сохранённого hash):
  «Войти через VK» → создать звонок → сохранить hash → можно выйти из аккаунта в UI

Далее каждый Connect (Path B):
  анонимный vkcalls + сохранённый hash
  → TURN creds (TCP) → WRAP → RAW
  (без повторного логина в VK, 1 worker/поток)
```

RAW без hash звонка не поднимается. Создание звонка — единственная роль аккаунта.

### Если звонок умер («не найден / закрыт»)

| Режим | Поведение |
|-------|-----------|
| **По умолчанию** | Сообщить пользователю и **спросить**: пересоздать (потребует VK-логин) / отмена |
| **Тихий режим** (настройка) | В фоне: при необходимости снова через сохранённую VK-сессию создать звонок, обновить hash, продолжить Connect — без диалогов |

### Транспорт и нагрузка

- До TURN-relay: **только TCP**
- **Workers = 1** (один поток на звонок; не раздувать как multi-worker qWDTT)

### Что видит пользователь

| Элемент | Поведение |
|---------|-----------|
| Звонок/Капча | Скрыто (всегда vkcalls) |
| Капча | Нет |
| Войти через VK | Чтобы создать/пересоздать звонок |
| Тихий режим | Настройка: авто-пересоздание звонка в фоне |
| Потоки/workers | Не показываем (фиксировано 1) |

### Связь с NetworkProbe

Preselect Bypass → Connect → anonymous `vkcalls(hash)` по TCP.  
Если hash нет — сначала VK-логин и создание звонка.  
Если hash мёртв — диалог или тихий recreate (по настройке).
---
...
```json
{
  ...
  "bypass": {
    ...
    "vk": {
      "dial": "vkcalls",
      "auth": "anonymous-after-hash",
      "createCallViaAccount": true,
...
4. Bypass: VK только для создания hash; Connect = anonymous vkcalls + TCP + 1 worker; диалог/тихий recreate при мёртвом звонке.
