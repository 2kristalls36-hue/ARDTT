...
На LTE Auto кнопка, виджет и плитка делят один запрос (`ConnectRequestCoordinator`). Ожидание принадлежит этой операции и серии: ранний ordinary-успех или финал текущего раунда завершает ожидание, продолжение вызывает запуск один раз и не заходит повторно в очередь. Яндекс/VK ожидание не завершают. Stop отзывает probe, ожидание и продолжение независимо от фазы UI; поздний callback другой серии не завершает новый запрос. Wi‑Fi Auto идёт сразу в Direct. Устаревший балл 80 не выбирает обход: нужен свежий сильный раунд.
