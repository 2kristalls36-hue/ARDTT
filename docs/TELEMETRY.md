...
2. **Рамка поверх окон приложения** — `RecordingFrameWindowHost` вешает пульсирующий контур через `WindowManager` (`LAST_SUB_WINDOW`), чтобы листы и диалоги его не перекрывали. Поверх чужих приложений и status bar по-прежнему нужен `SYSTEM_ALERT_WINDOW`.
