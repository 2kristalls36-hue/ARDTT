...
- **Хранилище** — файлы на устройстве: имя, дата начала, длительность, размер.
  У каждой записи в одной строке: поле комментария, **Отправить** и **Удалить**.
  Нажатие на поле открывает выезжающий лист (`ArdttDialog`), как у остальных диалогов приложения.
  В строке виден обрезанный черновик. Если комментарий пуст, **Отправить** открывает тот же лист.
- **История** — уже отправленные файлы. Номер `Обращение №N` выдаёт сервер при приёме.
  Здесь же видно, разобрал ли лог автор программы («Ожидает разбора» / «Прочитано»).
  Список обновляется при открытии вкладки, pull-to-refresh и каждые ~20 с.

После успешной отправки файл удаляется из хранилища, открывается **История**, и запись туда попадает.
Повторная загрузка того же имени файла на сервере сохраняет тот же номер и сбрасывает отметку «прочитан».
Комментарий встраивается в JSONL как событие `user_comment`.
...
Клиент читает свою историю без review-токена:

```
GET /api/logs/{client_id}/status
GET /api/logs/{client_id}/{filename}/status
```
...
### Очередь разбора, «прочитан» и ответ автора

Два шага, чтобы пользователь видел прогресс во вкладке **История**:

1. Как только агент взял файл в работу — пометить прочитанным. В приложении
   статус меняется на **«Прочитано»**.
2. Когда разбор закончен — записать ответ. Он появляется в карточке обращения
   блоком **«Ответ автора»**.

Найти непрочитанные или обращение по номеру (на VPS, review API):

```bash
curl -s 'http://127.0.0.1:9200/api/logs?status=unread' | jq
curl -s 'http://127.0.0.1:9200/api/logs/ticket/12' | jq
curl -s 'http://127.0.0.1:9200/api/logs?ticket=12' | jq
```

Шаг 1 — сразу показать, что файл прочитан (комментарий ещё не обязателен):
...
Шаг 2 — ответ, который увидит пользователь:

```bash
curl -s -X POST \
  -H 'Content-Type: application/json' \
  -d '{"processed_by":"cursor-agent","reply":"краткий итог разбора для пользователя"}' \
  'http://127.0.0.1:9200/api/logs/{client_id}/{filename}.json/comment'
```

Повторный `POST /read` без `reply`/`note` не стирает уже записанный ответ.
`POST /comment` сам ставит «прочитан», если этого ещё не сделали.

Рядом с логом создаётся скрытый sidecar
`.{filename}.json.read.json` с `processed_at`, `processed_by`, `reply` и
`replied_at`. Номер обращения живёт в `.{filename}.json.ticket.json` и в
счётчике `/var/logs/app/.ticket_seq`. Сам лог не изменяется после загрузки.
Повторная загрузка файла с тем же именем сбрасывает отметку «прочитан» и ответ,
но номер сохраняет.

Клиент забирает статус через `GET /api/logs/{client_id}/status` (без
review-токена) и показывает `read` и `reply` на вкладке «История».
...
telemetry/
  ...
  TestingTicketStore.kt     — черновики, локальный кэш истории, статус «прочитан»
  TelemetryUploadClient.kt  — POST, номер с сервера, inbox GET, прогресс, retry
...
2. **Рамка поверх окон приложения** — `RecordingFrameWindowHost` вешает пульсирующий контур через `WindowManager` (`TYPE_APPLICATION`), без inset status/navigation bar (`fitInsetsTypes = 0`), и поднимает его при каждой смене фокуса, чтобы листы и диалоги его не перекрывали. Поверх чужих приложений по-прежнему нужен `SYSTEM_ALERT_WINDOW`.
