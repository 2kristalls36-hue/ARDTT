...
заметкой. Номер обращения живёт в `.{filename}.json.ticket.json` и в счётчике
`/var/logs/app/.ticket_seq`. Сам лог не изменяется после загрузки. Повторная
загрузка файла с тем же именем сбрасывает отметку «прочитан», но номер
сохраняет.

Клиент забирает статус через `GET /api/logs/{client_id}/status` (без
review-токена) и показывает его на вкладке «История».
