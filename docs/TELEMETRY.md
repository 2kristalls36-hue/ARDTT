...
2. **Рамка поверх окон приложения** — `RecordingFrameWindowHost` вешает пульсирующий контур через `WindowManager` (`TYPE_APPLICATION`) и поднимает его при каждой смене фокуса, чтобы листы и диалоги его не перекрывали. Поверх чужих приложений по-прежнему нужен `SYSTEM_ALERT_WINDOW`.
