...
Живая проверка на **белом списке оператора** (LTE, не эмулятор): слои юнит-тестов / этой телеметрии / USB-`adb` — [DEVICE-TEST.md](DEVICE-TEST.md). Облачный агент Cursor USB телефона не видит; кабель работает только на машине, где запущен `adb` (локальный агент или self-hosted worker).
...
- Нажатие на поле открывает меню, где пишется комментарий. В самой строке виден обрезанный текст.
- Если комментарий пуст, **Отправить** открывает то же меню вместо отдельного окна.
- Каждое обращение получает порядковый номер (`Обращение №N`). История номеров и комментариев хранится на устройстве. Повторная отправка того же лога оставляет тот же номер.
- Комментарий и номер встраиваются в JSONL как событие `user_comment`.
...
telemetry/
  ...
  TestingTicketStore.kt     — черновик комментария, номера и история обращений
