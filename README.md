...
> [!NOTE]
> Клиент **0.5.265** (`versionCode` 286), пакет `com.ardtt.app`. Серверный стек **1.0.53** (`DEPLOY_VERSION`, каталог `/opt/ardtt`). Канонический источник стека — актив Releases, не `assets/` APK. Старые APK (ждут `ardtt-stack-*.tar.gz` / `main`) этот пакет не ставят.
>
