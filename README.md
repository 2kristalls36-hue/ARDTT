...
> [!NOTE]
> Клиент **0.5.236** (`versionCode` 254), пакет `com.ardtt.app`. Серверный стек **1.0.35** (`DEPLOY_VERSION`, каталог `/opt/ardtt`).
>
...
| | |
|---|---|
| Клиент | **0.5.236** · minSdk 28 · APK `arm64-v8a` / `armeabi-v7a` / `x86_64` / universal · [Releases](https://github.com/2kristalls36-hue/ARDTT/releases) |
...
Пока репозиторий **приватный**, с VPS без токена clone не выйдет — ставьте из приложения. Когда сделаете репозиторий **публичным**, достаточно клона тега: архив в APK для сервера больше не обязателен. Имеет смысл клонировать **тег** `v0.5.236` (стек **1.0.35**), а не скользящий `main`.
...
```bash
git clone --depth 1 --branch v0.5.236 \
...
| Документ | Содержание |
|----------|------------|
| [CHANGELOG.md](CHANGELOG.md) | Линейка 0.5.236 / стек 1.0.35 |
