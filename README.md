...
> [!NOTE]
> Клиент **0.5.251-test** (`versionCode` 269), пакет `com.ardtt.app`. Серверный стек **1.0.40** (`DEPLOY_VERSION`, каталог `/opt/ardtt`). Канонический источник стека — этот репозиторий, не `assets/` APK.
>
...
| | |
|---|---|
| Клиент | **0.5.251-test** · minSdk 28 · APK `arm64-v8a` / `armeabi-v7a` / `x86_64` / universal · [Releases](https://github.com/2kristalls36-hue/ARDTT/releases/latest) |
| Стек | **1.0.40** · `/opt/ardtt` · контейнер `ardtt` (isolated netns) · переменные `ARDTT_*` |
| Откуда стек | GitHub Releases `ardtt-stack-1.0.40.tar.gz` или архив тега — **не** APK |
...
| Документ | Содержание |
|----------|------------|
| [CHANGELOG.md](CHANGELOG.md) | Линейка 0.5.251-test / стек 1.0.40 |
