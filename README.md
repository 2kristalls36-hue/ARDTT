...
> [!NOTE]
> Клиент **0.5.243** (`versionCode` 261), пакет `com.ardtt.app`. Серверный стек **1.0.37** (`DEPLOY_VERSION`, каталог `/opt/ardtt`). Канонический источник стека — этот репозиторий, не `assets/` APK.
>
...
| | |
|---|---|
| Клиент | **0.5.243** · minSdk 28 · APK `arm64-v8a` / `armeabi-v7a` / `x86_64` / universal · [Releases](https://github.com/2kristalls36-hue/ARDTT/releases/latest) |
| Стек | **1.0.37** · `/opt/ardtt` · контейнер `ardtt` (isolated netns) · переменные `ARDTT_*` |
| Откуда стек | GitHub Releases `ardtt-stack-1.0.37.tar.gz` или архив тега `v0.5.243` — **не** APK |
...
Клонируйте **тег** `v0.5.243` (стек **1.0.37**), а не скользящий `main`. Репозиторий публичный: HTTPS clone, `raw.githubusercontent.com` и Releases **не требуют** секретов.
...
```bash
git clone --depth 1 --branch v0.5.243 \
...
| Документ | Содержание |
|----------|------------|
| [CHANGELOG.md](CHANGELOG.md) | Линейка 0.5.243 / стек 1.0.37 |
