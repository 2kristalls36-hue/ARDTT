...
> [!NOTE]
> Клиент **0.5.267** (`versionCode` 288), пакет `com.ardtt.app`. Серверный стек **1.0.53** (`DEPLOY_VERSION`, каталог `/opt/ardtt`). Канонический источник стека — актив Releases, не `assets/` APK. Старые APK (ждут `ardtt-stack-*.tar.gz` / `main`) этот пакет не ставят.
>
...
| | |
|---|---|
| Клиент | **0.5.267** · minSdk 28 · APK `arm64-v8a` / `armeabi-v7a` / `x86_64` / universal · [Releases](https://github.com/2kristalls36-hue/ARDTT/releases/latest) |
...
| Документ | Содержание |
|----------|------------|
| [CHANGELOG.md](CHANGELOG.md) | Линейка 0.5.267 / стек 1.0.53 |
