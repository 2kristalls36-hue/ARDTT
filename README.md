...
> [!NOTE]
> Клиент **0.5.267** (`versionCode` 289), пакет `com.ardtt.app`. Серверный стек **1.0.54** (`DEPLOY_VERSION`, каталог `/opt/ardtt`). Канонический источник стека — актив Releases, не `assets/` APK. Старые APK (ждут `ardtt-stack-*.tar.gz` / `main`) этот пакет не ставят.
>
...
| Документ | Содержание |
|----------|------------|
| [CHANGELOG.md](CHANGELOG.md) | Линейка 0.5.267 / стек 1.0.54 |
