...
> [!NOTE]
> Клиент **0.5.268** (`versionCode` считается CI из истории git, ≈ число коммитов × 10), пакет `com.ardtt.app`. Серверный стек **1.0.54** (`DEPLOY_VERSION`, каталог `/opt/ardtt`). Канонический источник стека — актив Releases, не `assets/` APK. Старые APK (ждут `ardtt-stack-*.tar.gz` / `main`) этот пакет не ставят.
>
...
| | |
|---|---|
| Клиент | **0.5.268** · minSdk 28 · APK `arm64-v8a` / `armeabi-v7a` / `x86_64` / universal · [Releases](https://github.com/2kristalls36-hue/ARDTT/releases/latest) |
...
| Документ | Содержание |
|----------|------------|
| [CHANGELOG.md](CHANGELOG.md) | Линейка 0.5.268 / стек 1.0.54 |
