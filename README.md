...
> [!NOTE]
> Клиент **0.5.265** (`versionCode` 285), пакет `com.ardtt.app`. Серверный стек **1.0.53** (`DEPLOY_VERSION`, каталог `/opt/ardtt`). Канонический источник стека — актив Releases, не `assets/` APK. Старые APK (ждут `ardtt-stack-*.tar.gz` / `main`) этот пакет не ставят.
>
...
| Документ | Содержание |
|----------|------------|
| [CHANGELOG.md](CHANGELOG.md) | Линейка 0.5.265 / стек 1.0.53 |
