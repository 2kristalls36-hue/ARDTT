...
> [!NOTE]
> Клиент **0.5.238** (`versionCode` 256), пакет `com.ardtt.app`. Серверный стек **1.0.35** (`DEPLOY_VERSION`, каталог `/opt/ardtt`).
>
...
Пока репозиторий **приватный**, с VPS без токена clone не выйдет — ставьте из приложения. Когда сделаете репозиторий **публичным**, достаточно клона тега: архив в APK для сервера больше не обязателен. Имеет смысл клонировать **тег** `v0.5.238` (стек **1.0.35**), а не скользящий `main`.
...
| Документ | Содержание |
|----------|------------|
| [CHANGELOG.md](CHANGELOG.md) | Линейка 0.5.238 / стек 1.0.35 |
