...
ins="$("$bin" emit error --code INSUFFICIENT_DISK --message 'Мало места' --filesystem /opt/ardtt --required-bytes 10 --available-bytes 1)"
echo "$ins" | grep -q 'ARDTT_ERROR|code=INSUFFICIENT_DISK|Мало места' || { echo "FAIL: insufficient disk"; exit 1; }
echo "$ins" | grep -q '"requiredBytes":10' || { echo "FAIL: requiredBytes json"; exit 1; }
