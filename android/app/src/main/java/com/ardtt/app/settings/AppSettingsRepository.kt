package com.ardtt.app.settings
...
import com.ardtt.app.core.sanitizeTrustedWifiSsid
import java.security.MessageDigest
import kotlinx.coroutines.flow.Flow
import kotlinx.coroutines.flow.first
import kotlinx.coroutines.flow.map

private val Context.dataStore: DataStore<Preferences> by preferencesDataStore("ardtt_settings")
...
class AppSettingsRepository(private val context: Context) {
    ...
    suspend fun clearLegacyWallpaperPreference() {
        context.dataStore.edit {
            ...
            it.remove(stringPreferencesKey("alpha_challenge_hex"))
            it.remove(booleanPreferencesKey("alpha_unlocked"))
            it.remove(intPreferencesKey("alpha_unlock_fails"))
            it.remove(longPreferencesKey("alpha_unlock_lock_until"))
        }
    }
